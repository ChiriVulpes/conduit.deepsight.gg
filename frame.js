...
void (async () => {
    ...
    function resend() {
        if (!unresolvedCalls.size)
            return;
        log(`%c${new Date().toTimeString().slice(0, 8)} %cconduit.deepsight.gg %c/`, PUNCT_FORMAT, MAIN_FORMAT, PUNCT_FORMAT, `Service updated. ${!unresolvedCalls.size ? 'No calls to resend' : `Resending ${unresolvedCalls.size} unresolved messages`}`);
