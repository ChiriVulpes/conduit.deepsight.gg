...
void (async () => {
    ...
    navigator.serviceWorker.addEventListener('message', event => {
        ...
        if (id !== 'global' && !unresolvedMessages.has(id)) {
            log(`%c${new Date().toTimeString().slice(0, 8)} %cconduit.deepsight.gg %c/ %cHost %c\u2B9C Service %c/ %cReceived response for unknown message ID %o %o${printIfVerbose()}`, PUNCT_FORMAT, MAIN_FORMAT, PUNCT_FORMAT, HOST_FORMAT, SERVICE_FORMAT, PUNCT_FORMAT, ERROR_FORMAT, id, type, ...ifVerbose(data));
            return;
        }
    ...
    function resend() {
        if (!unresolvedMessages.size)
            return;
        log(`%c${new Date().toTimeString().slice(0, 8)} %cconduit.deepsight.gg %c/`, PUNCT_FORMAT, MAIN_FORMAT, PUNCT_FORMAT, 'Service updated, resending unresolved messages');
